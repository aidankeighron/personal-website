...
export default function VideoEntry() {
  ...
  useLayoutEffect(() => {
    if (hasMounted && videoRef.current) {
      const handleResize = () => {
        if (videoRef.current && videoRef.current.offsetHeight > window.innerHeight) {
  ...
  const videoUrl = 'https://firebasestorage.googleapis.com/v0/b/personal-website-54361.appspot.com/o/Introvideo10-opt.mp4?alt=media&token=36f5d109-97c1-4d8e-95ad-455e342c1baa';
  const fallbackUrl = "/videos/introvideo10.mp4";

  if (!hasMounted) {
    return (
      <div className="relative w-full aspect-video bg-black flex items-center justify-center">
        <h1 className='text-main text-4xl hidden sm:block'>Aidan Keighron</h1>
      </div>
    );
  }
    ...
    <div className='w-full flex flex-col items-center'>
      ...
      <div className='relative w-full overflow-hidden'>
        <video 
          ...
          style={{marginTop: `-${videoTopOffset}px` }}
        >
