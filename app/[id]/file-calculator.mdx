...
The code shown here has been modified to make it easier to explain. If you would like to check it out, the full code is available on GitHub: <a href="https://github.com/aidankeighron/calculator" target="_blank">Calculator</a>
...
<CodeBlock code={`public int add(int x1, int x2) {
    return x1 + x2;
...
<CodeBlock code={`// Get answer
File numberFile = new File(firstNumber + "/" + secondNumber);
System.out.println("Your answer is: " + numberFile.list()[0]);`} language="language-java"/>
...
<CodeBlock code={`public static class CalcThread extends Thread {
...
<CodeBlock code={`private static void calculateNumbers(int numThreads) throws Exception {        
...
<CodeBlock code={`public static void deleteNumbers(int numThreads) throws Exception {
    int rowPerThread = maxNum/numThreads;
...
Full code: <a href="https://github.com/aidankeighron/calculator" target="_blank">Calculator</a>
../components/physics/CodeBlock