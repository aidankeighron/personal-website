...
The code shown here has been modified to make it easier to explain. If you would like to check it out, the full code is available on GitHub: <a href="https://github.com/aidankeighron/sheet-scraper" target="_blank">Sheet Scraper</a>. If you would like to see the final result, here is a link to the <a href="https://docs.google.com/spreadsheets/d/1xDtfRs81EcdFHOiG-Nua_mBz2xAD4tFVVtNt6SjwPoM/edit#gid=341185536" target="_blank">spreadsheet</a>.
...
- <a href="https://www.thebluealliance.com/apidocs" target="_blank">Blue alliance API key (optional)</a>

Blue alliance is used to get the name of a team when only the number has been provided
- <a href="https://docs.discordbotstudio.org/setting-up-dbs/finding-your-bot-token" target="_blank">Discord bot token</a>

A discord bot is used to scrape the spreadsheet data from the FRC Shirt Trading discord server
- <a href="https://developers.google.com/sheets/api" target="_blank">Account credentials from a google cloud project with access to google sheets API</a>
...
<CodeBlock code={`def retrieve_messages(channelID):
    headers = {
...
def get_ids():
    return get_sheet_id(retrieve_messages(keys.CHANNEL_ID))`} language="language-python"/>
...
<CodeBlock code={`client = discord.Client(intents=discord.Intents.default())
messages = []
users = []
...
def get_forum_ids():
    ...
    return (messages, users)`} language="language-python"/>
...
<CodeBlock code={`def get_sheet(id):
    ...
    if "values" in result:
        ...
        return -1`} language="language-python"/>
...
<img src="/images/sheet-scraper/example_spreadsheet.PNG" style={{'border':"5px solid black", 'display': 'block', 'margin-left': 'auto', 'margin-right': 'auto', 'width': '80%'}}></img>
...
for _ in range(10):
    ...
    if categoriesFound >= MIN_CATEGORIES:
        break`} language="language-python"/>

Let's break it down. First, we initialize a `start` variable that we will use to keep track of where the spreadsheet starts.

<CodeBlock code={`category_locations = {category:-1 for category in CATEGORIES}
for row in range(start + 1, len(sheet)):
    for col in range(len(sheet[row])):`} language="language-python"/>
...
<CodeBlock code={`def check_category(val):
    if val == "":
        return "none"
    val = val.lower()
    ...
    if "team" in val:
        return "Number"
    return "none"`} language="language-python"/>
...
<CodeBlock code={`if check_category(sheet[row][col]) != "none" and category_locations[check_category(sheet[row][col])] == -1:
    category_locations[check_category(sheet[row][col])] = col
    start = row`} language="language-python"/>
...
<CodeBlock code={`for row in range(start + 1, len(sheet)):
    for col in range(len(sheet[row])):
    if start == row:
        break`} language="language-python"/>
...
<CodeBlock code={`categoriesFound = sum(1 for _, category in category_locations.items() if category != -1)
if categoriesFound >= MIN_CATEGORIES:
    break`} language="language-python"/>
...
<CodeBlock code={`for _ in range(10):`} language="language-python"/>
...
<CodeBlock code={`def parse_sheet(sheet, start, end, user, id, category_locations):
    shirts = []
    empty = 0
    for row in range(start+1, len(sheet) if end == -1 else end):
        ...
        if empty > MAX_EMPTY:
            break
    return shirts`} language="language-python"/>
...
<CodeBlock code={`for row in range(start+1, len(sheet) if end == -1 else end):
    shirt = {category:"" for category in CATEGORIES}
    shirt["User"] = user
    shirt["ID"] = id`} language="language-python"/>
...
<CodeBlock code={`for category, col in category_locations.items():
    if col == -1 or col >= len(sheet[row]):
        continue
    shirt[category] = sheet[row][col]`} language="language-python"/>
...
<CodeBlock code={`if shirt["Name"] == "" and shirt["Number"] != "":
    try:
        shirt["Name"] = get_team_name(int(shirt["Number"]))
    except:
...
<CodeBlock code={`def get_team_name(number):
    try:
    ...
    return request.json()['nickname']`} language="language-python"/>
...
<CodeBlock code={`numNotEmpty = sum(1 for _, col in shirt.items() if col != '')
if numNotEmpty >= MIN_SHIRT_DATA:
    shirts.append(shirt)
    empty = 0
empty += 1
...
<CodeBlock code={`def sort_sheet(shirts):
    nonInt = {}
...
<CodeBlock code={`for row in shirts:
    try:
        row[1] = int(row[1])
    except:
        nonInt[str(currentInt)] = row[1]  
        row[1] = currentInt
        currentInt += 1`} language="language-python"/>
...
<CodeBlock code={`shirts.sort(key=lambda x: int(x[1]))
...
for row in shirts:
    try:
        ...
        print(str(e))`} language="language-python"/>
...
<CodeBlock code={`def write_result(sheet, id):
    ...
    return result`} language="language-python"/>
...
<CodeBlock code={`sheet = get_sheet(RESULT)
numberOfShirts = {}
del sheet[0]
for shirt in sheet:
...
result = service.spreadsheets().values().update(spreadsheetId=RESULT, range="Count", valueInputOption="RAW", body=values).execute()`} language="language-python"/>
...
Full code: <a href="https://github.com/aidankeighron/sheet-scraper" target="_blank">Sheet Scraper</a> Result: <a href="https://docs.google.com/spreadsheets/d/1xDtfRs81EcdFHOiG-Nua_mBz2xAD4tFVVtNt6SjwPoM/edit#gid=341185536" target="_blank">Spreadsheet</a>../components/physics/CodeBlock
