  ...
  return (
    <>
      ...
      <div className='flex flex-col items-center pt-40'>
        ...
        <h1 className='text-sm text-second dark:text-a-main mb-10'>*Check out the playground below</h1>
          ...
          {badConflictRobots.map((robot) => {
            return (
              <div key={robot.name} className='mx-16 mb-16 flex flex-row odd:flex-row-reverse justify-between'>
                {robot.image1 && <Image width={200} height={200} src={robot.image1} alt={robot.image1Alt ?? ""} className='w-1/4 rounded-xl' />}
                <div className='self-center mx-12'>
                  <div className='flex flex-row items-end mb-5'>
                    ...
                    <h4 className='text-lg text-second dark:text-d-second'>W: {robot.record.w}, L: {robot.record.l}</h4>
                  </div>
                    ...
                    {Object.entries(robot.info).map(([key, value]) => {
                      return (
                        <div key={key} className='flex flex-row mb-1'>
                          ...
                          <p className='text-lg font-light dark:font-semibold text-second dark:text-a-main'>{value}</p>
                        </div>
                      );
                    })}
                  </div>
                </div>
                {robot.image2 && <Image width={200} height={200} src={robot.image2} alt={robot.image2Alt ?? ""} className='w-1/4 rounded-xl' />}
              </div>
            )
          })}
          </div>
          <h1 className='text-3xl bottom-border font-medium w-fit mb-2'>Playground</h1>
          <p className='text-second dark:text-d-second text-base mb-10'>* W A S D to move | R to reset</p>
          <StrictMode>
